...
export async function OptimizationMatrixScreen({ state }) {
    const buildLevel = getBuildLevel(state);
  const selected = String(getPickedAtLevel(state, "optimizationMatrix") ?? "");
  const need = Math.max(0, Number(state.ui?.picker?.need || 0)) || 1;
  const subtitle = `Level ${buildLevel} • Pick ${need} (${selected ? 1 : 0}/${need})`;

  const items = await loadPickListItems("optimization_matrix");
