...
function filterBar(total){
  // Inline handler so it works with innerHTML re-renders.
  const oninput = `
    (function(el){
      try{
...
export function OptimizationMatrixScreen({ state }) {
  const selected = state.character.optimizationMatrix;

  // Placeholder options until a data-driven list is added.
  const options = [
    { id: "opt-a", label: "Optimization A", icon: "⚙️", desc: "" },
    { id: "opt-b", label: "Optimization B", icon: "⚙️", desc: "" },
  ];
  ...
  const rows = options.map(o => {
    ...
    return `
      <button class="card compact-row ${isOn ? "selected" : ""}"
              ...
              data-action="select-optimizationMatrix" data-id="${escapeHtml(o.id)}">
  ...
  return `
    <div class="screen picker-screen">
      <div class="h1">Optimization Matrix</div>
      <div class="h2">Choose matrix</div>
