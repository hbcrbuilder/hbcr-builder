...
export function ManoeuvresScreen({ state }) {
  const selected = state.character.manoeuvres;

  const options = [
    { id: "riposte", label: "Riposte", icon: "↩️", desc: "" },
    { id: "trip", label: "Trip Attack", icon: "🦶", desc: "" },
    { id: "precision", label: "Precision Attack", icon: "🎯", desc: "" },
  ];
  ...
  const rows = options.map(o => {
    ...
    return `
      <button class="card compact-row ${isOn ? "selected" : ""}"
              ...
              data-action="select-manoeuvres" data-id="${escapeHtml(o.id)}">
  ...
  return `
    <div class="screen picker-screen">
      <div class="h1">Manoeuvres</div>
      <div class="h2">Choose manoeuvres</div>
